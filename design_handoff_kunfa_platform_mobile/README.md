...
A mobile companion is included (`Kunfa Mobile.html` + `mobile-styles.css` + `m-*.jsx`). Same design system, restructured for a phone screen.

### Mobile layout
- Frame: 390×844 (iPhone 14/15 reference). Safe area: 50px top status bar, 84px bottom tab bar.
- Outer page padding: **20px horizontal**, content starts at 8–12px below nav header.
- Sidebar is **replaced by a 5-tab bottom bar**: Home, Deals, Community, Invites, Profile. The full desktop nav (Marketplace, Startups, Investors, FAQ, Founder/Investor flows) is reachable via Profile + deep links — keep those routable.

### Mobile primitives
- **`NavHeader`** — left: serif page title (22px) OR back button (chev + label). Right: 36×36 sunk-bg circular icon buttons (search, bell). 8/20/12 padding.
- **Tab bar** — `rgba(255,255,255,0.92)` bg with `backdrop-filter: blur(20px)`, top 1px line, 5 equal-width slots, 22px stroked icon over 10px label. Active item: ink color + 4×4 accent dot above icon. Bottom safe-area inset of 22px.
- **`SearchBar`** — sunk-bg pill, 10px radius, 10×12 padding, leading search icon, 13px placeholder. Placed below NavHeader on listing screens.
- **`StatCard` (mobile)** — same shell as desktop but 14px radius, 14×14×16 padding, 26px serif value (down from 34px). Mobile uses 2-up grid, 10px gap.
- **`Pill tabs`** — horizontal-scroll row replacing desktop's underline tabs. Each pill: 7×14 padding, 99px radius, 12.5px medium. Inactive: sunk-bg + ink-soft text. Active: charcoal-fill + light text.
- **`DealRow` (mobile)** — `38px 1fr auto` grid, 14×16 padding, 14px radius. Logo, name + meta, score pill (`tag-accent` style).
- **`Stepper` (mobile)** — horizontal-scroll inline strip inside a card. Tighter chip (18px) and smaller labels.
- **Form fields (mobile)** — input height ~44px (15px text + 12×14 padding), 10px radius. Labels are 11px uppercase 0.08em. Match iOS hit-target minimums.
- **Form submit** — full-width charcoal, 15px padding, 12px radius, 14px semibold. Use `btn-m-accent` for the second invitation card only.
- **`HeroCard`** — full-bleed dark card with corner amber radial glow. Used for "$100 sourcing credit" strip on Home and the "Earn $50 per referral" banner on Invites. 14–18px radius, 22px padding.

### Mobile screens (in order)
1. **Home** — date eyebrow, serif greeting, $100 credit hero strip, 2×2 stat grid, "For you today" deal list (top 3 by score).
2. **Deals** — search bar, pill tabs (All / For you / Closing soon / Score 80+ / Saved), full deal list.
3. **Community** — pill tabs (Discussions / Events / Sense Check / Screen a Startup), AI-Matching dark card, threads list with smaller avatars + truncated meta.
4. **Invitations** — referral banner with 3-up stat triplet, two stacked invite cards (Refer-an-Investor uses accent CTA), info callout, history empty state.
5. **Founder Onboarding** — back-arrow header, icon + title head, horizontal stepper, single-column form (sector/stage are paired), full-width submit.
6. **Join as Investor** — same form pattern, shield icon, accreditation select required.
7. **FAQ** — accordion list (single open at a time), "Still have questions?" charcoal CTA card with white pill button.

### Mobile interaction notes
- All pill-tab rows scroll horizontally with momentum; hide scrollbars.
- Tab-bar active state should also drive haptic feedback (`UIImpactFeedbackGenerator.light`) on iOS.
- Form fields trigger native keyboards (`type="email"`, `type="url"`, numeric `inputmode` on check sizes).
- Bottom safe-area inset: `env(safe-area-inset-bottom)` should be added to the tab bar's bottom padding.
- Pull-to-refresh on Home, Deals, Community lists.
- Sticky NavHeader is acceptable; otherwise it scrolls naturally.

### Responsive bridge (tablet / split)
At ≥768px, switch to the desktop sidebar layout. There's no "tablet-specific" version — the 248px sidebar + main content reads fine down to ~768px. Below that, snap to mobile.
...
**Mobile:**
- `Kunfa Mobile.html` — mobile entry point (canvas of all 7 phone frames side-by-side)
- `mobile-styles.css` — mobile-specific styles + tokens
- `m-shell.jsx` — Phone frame, StatusBar, TabBar, NavHeader
- `m-screens.jsx` — all 7 mobile screen layouts
- `m-app.jsx` — canvas layout

...
export default function AddCompanyPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto py-8 px-4">
      <h1 className="text-2xl font-bold text-white mb-6">Add Company</h1>
      ...
      {tab === 'pdf' && !form.company_name && (
        <div className="mb-6">
          <label
            htmlFor="pdf-upload"
            className="flex flex-col items-center justify-center w-full h-48 border-2 border-dashed border-gray-600 rounded-xl hover:border-blue-500 transition cursor-pointer bg-gray-800/50"
          >
            {extracting ? (
              <>
                <div className="w-8 h-8 border-2 border-blue-400 border-t-transparent rounded-full animate-spin mb-3" />
                <p className="text-sm text-gray-300">Extracting details from PDF...</p>
              </>
            ) : (
              <>
      ...
      {(tab === 'manual' || form.company_name) && (
        <form onSubmit={handleSubmit} className="space-y-4">
          {tab === 'pdf' && form.company_name && (
            <div className="bg-blue-900/20 border border-blue-800 rounded-lg p-3 mb-2">
              <p className="text-sm text-blue-300">Fields extracted from PDF. Review and edit as needed.</p>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-1">Raise Amount (USD)</label>
              <input
                type="number"

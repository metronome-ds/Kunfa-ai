...
export default function InvestorsDirectoryPage() {
  ...
  if (!isTenantContext) {
    return (
      <div className="p-8 max-w-4xl mx-auto text-center">
        <h1 className="text-xl font-semibold text-[var(--ink)]">Investor Directory</h1>
  ...
  if (!hasFeature) {
    return (
      <div className="p-8 max-w-4xl mx-auto text-center">
        ...
        <p className="text-sm text-[var(--ink-mute)] mt-2">The investor directory is not enabled for this tenant.</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto">
      ...
      <div className="bg-[var(--bg-elev)] rounded-xl border border-[var(--line)] p-4 mb-6 flex flex-wrap gap-3 items-center">
        <div className="relative flex-1 min-w-[200px]">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--ink-faint)]" />
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search name or firm..." className="w-full pl-9 pr-3 py-2 border border-[var(--line)] rounded-lg text-sm focus:outline-none focus:shadow-[var(--focus-ring)] focus:border-[var(--line-strong)]" />
        </div>
        <select value={sector} onChange={(e) => setSector(e.target.value)} className="px-3 py-2 border border-[var(--line)] rounded-lg text-sm">
      ...
      {loading ? (
        ...
        <div className="bg-[var(--bg-elev)] rounded-xl border border-[var(--line)] p-12 text-center">
          <Users className="w-10 h-10 text-[var(--ink-faint)] mx-auto mb-3" />
          <h2 className="text-lg font-semibold text-[var(--ink)]">No investors yet</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {investors.map((inv) => {
            ...
            return (
              <div key={inv.user_id} className="bg-[var(--bg-elev)] rounded-xl border border-[var(--line)] p-5">
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      <h3 className="font-semibold text-[var(--ink)] truncate">{inv.full_name || 'Investor'}</h3>
                      {inv.is_accredited && <CheckCircle2 className="w-4 h-4 text-emerald-500 flex-shrink-0" />}
                    </div>
                    {inv.job_title && <p className="text-xs text-[var(--ink-mute)] truncate">{inv.job_title}</p>}
                    {firm && <p className="text-xs font-medium text-[var(--ink-soft)] mt-0.5 truncate">{firm}</p>}
                  </div>
                  {inv.linkedin_url && (
                    <a href={inv.linkedin_url} target="_blank" rel="noreferrer" className="text-[var(--ink-faint)] hover:text-[#0077B5] flex-shrink-0">
                ...
                {inv.sector_interests && inv.sector_interests.length > 0 && (
                  <div className="flex flex-wrap gap-1 mb-2">
                    ...
                    {inv.sector_interests.length > 3 && <span className="text-xs text-[var(--ink-faint)]">+{inv.sector_interests.length - 3}</span>}
                  </div>
                )}

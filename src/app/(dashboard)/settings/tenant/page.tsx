...
const INPUT_CLASS = 'w-full rounded-lg border border-[var(--line-strong)] px-4 py-2.5 text-sm text-[var(--ink)] focus:border-[var(--line-strong)] focus:shadow-[var(--focus-ring)] outline-none transition-all'
...
export default function TenantSettingsPage() {
  ...
  if (!isTenantContext) {
    return (
      <div className="p-8 text-center">
        <p className="text-[var(--ink-mute)]">Tenant settings are only available in a tenant context.</p>
      </div>
    )
  }
  ...
  return (
    <div className="p-8 max-w-3xl mx-auto">
      ...
      {/* Tabs */}
      <div className="flex gap-1 mb-8 bg-[var(--bg-sunk)] rounded-lg p-1">
        {tabs.map(tab => (
          <button
            key={tab.id}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-[var(--bg-elev)] rounded-xl border border-[var(--line)] p-6 space-y-4">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-sm text-[var(--ink-soft)]">Show &quot;Powered by Kunfa&quot; badge</span>
            </label>
          </div>
        </div>
      )}
      ...
      {activeTab === 'access' && (
        <div className="bg-[var(--bg-elev)] rounded-xl border border-[var(--line)] p-6 space-y-4">
          <h2 className="text-lg font-semibold text-[var(--ink)]">Access Control</h2>
          <div>
          ...
          <div className="space-y-3 pt-2">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={form.require_accreditation === true} onChange={(e) => updateField('require_accreditation', e.target.checked)} className="h-4 w-4 rounded border-[var(--line-strong)] text-[var(--accent-ink)]" />
              <span className="text-sm text-[var(--ink-soft)]">Require accreditation verification</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={form.require_nda === true} onChange={(e) => updateField('require_nda', e.target.checked)} className="h-4 w-4 rounded border-[var(--line-strong)] text-[var(--accent-ink)]" />
              <span className="text-sm text-[var(--ink-soft)]">Require NDA before access</span>
            </label>
          </div>
          {form.require_nda && (
            <div>
              <label className="block text-sm font-medium text-[var(--ink-soft)] mb-1">NDA Document URL</label>
      ...
      {activeTab === 'legal' && (
        <div className="bg-[var(--bg-elev)] rounded-xl border border-[var(--line)] p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-[var(--ink-soft)] mb-1">Terms of Service URL</label>

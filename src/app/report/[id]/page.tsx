  ...
  const fallbackNav = (
    <nav className="bg-[var(--bg-elev)] border-b border-[var(--line)] px-6 py-4">
      <div className="max-w-6xl mx-auto flex items-center justify-between">
        ...
        <Link href="/dashboard" className="text-sm text-[var(--ink-mute)] hover:text-[var(--ink)] transition">
          Dashboard
        </Link>
      </div>
    </nav>
      ...
      <main className="max-w-6xl mx-auto px-6 py-8">
        ...
        {status === 'paid' && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-2xl font-bold font-[family-name:var(--serif)] tabular-nums tracking-tight text-[var(--ink)]">Kunfa Readiness Report</h1>
                <p className="text-sm text-[var(--ink-mute)] mt-1">Your AI-powered investment memorandum</p>
              </div>
              <button
                onClick={handleDownload}
                disabled={downloading}
                className="inline-flex items-center gap-2 px-5 py-2.5 bg-[var(--ink)] text-white rounded-lg font-semibold text-sm hover:bg-[var(--ink-2)] transition disabled:opacity-50"
              >
        ...
        {status === 'generating' && (
          <div className="text-center py-20">
            ...
            <h2 className="text-xl font-bold text-[var(--ink)] mb-2">Generating Your Report...</h2>
            <p className="text-[var(--ink-mute)] text-sm">Building your investment memorandum. This may take a moment.</p>
          </div>
        )}

        {/* Not Paid — Upsell */}
        {status === 'unpaid' && (
          <div className="w-full md:max-w-lg mx-auto text-center py-16">
            <div className="w-20 h-20 bg-[var(--accent-soft)] rounded-full flex items-center justify-center mx-auto mb-6">
              <svg className="w-10 h-10 text-[var(--accent-ink)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <h1 className="text-2xl font-bold font-[family-name:var(--serif)] tabular-nums tracking-tight text-[var(--ink)] mb-3">Unlock Your Full Readiness Report</h1>
            ...
            <button
              ...
              className="bg-[var(--ink)] text-white px-8 py-4 rounded-xl font-semibold text-base hover:bg-[var(--ink-2)] transition"
            >
              Unlock Report — $59
            </button>
            <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-3 text-left max-w-sm mx-auto">
              {[
                'Executive Summary',
                ...
                <div key={item} className="flex items-center gap-2">
                  <svg className="w-4 h-4 text-[var(--accent-ink)] flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                  </svg>
                  <span className="text-xs text-[var(--ink-soft)]">{item}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {status === 'error' && (
          <div className="w-full md:max-w-lg mx-auto text-center py-16">
            ...
            <h2 className="text-xl font-bold text-[var(--ink)] mb-2">Something went wrong</h2>
            <p className="text-[var(--ink-mute)] text-sm mb-6">{errorMessage}</p>
            <div className="flex items-center justify-center gap-4">
              ...
              <Link href="/dashboard" className="text-[var(--accent-ink)] font-semibold text-sm hover:underline">
                Go to Dashboard
              </Link>
            </div>
          </div>

    ...
    <div className="w-64 bg-[var(--bg-elev)] border-r border-[var(--line)] p-6 space-y-6 overflow-y-auto">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          {activeFilterCount > 0 && (
            <span className="px-2 py-1 bg-[var(--accent-soft)] text-[var(--accent-ink)] text-xs font-semibold rounded-full">
              {activeFilterCount}
            </span>
          )}
        </div>
        ...
        <label className="flex items-center justify-between gap-3 cursor-pointer">
          <span className="text-sm font-medium text-[var(--ink-soft)] flex items-center gap-1.5">
          ...
          <button
            ...
            className={`relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-[var(--ink)]/30 focus:ring-offset-2 ${
              filters.raisingOnly ? 'bg-emerald-500' : 'bg-gray-300'
            }`}
          >
            <span
              className={`inline-block h-3.5 w-3.5 transform rounded-full bg-[var(--bg-elev)] shadow transition-transform ${
        ...
        {/* Sort */}
        <div>
          <label className="text-sm font-medium text-[var(--ink-soft)] mb-2 block">Sort By</label>
          <select
            value={filters.sort}
            onChange={(e) => updateFilter({ sort: e.target.value as CompanyFilterState['sort'] })}
            className="w-full px-3 py-2 border border-[var(--line-strong)] rounded-lg text-sm text-[var(--ink)] focus:outline-none focus:ring-1 focus:ring-[var(--ink)]"
          >
      ...
      <div className="space-y-3">
        <button onClick={() => toggleSection('industry')} className="flex items-center justify-between w-full">
          <h4 className="font-semibold text-[var(--ink)]">Sector</h4>
          <span className={`text-[var(--ink-faint)] transition-transform ${expandedSections.industry ? 'rotate-180' : ''}`}>
            ▼
          </span>
        </button>
            ...
            {sortedIndustries.map((industry) => (
              <label key={industry} className="flex items-center gap-3 cursor-pointer">
                <input
                  ...
                  className="w-4 h-4 text-[var(--accent-ink)] rounded border-[var(--line-strong)] cursor-pointer"
                />
                <span className="text-sm text-[var(--ink-soft)] flex items-center gap-1.5">
                  {industry}
      ...
      <div className="space-y-3">
        <button onClick={() => toggleSection('stage')} className="flex items-center justify-between w-full">
          ...
          <span className={`text-[var(--ink-faint)] transition-transform ${expandedSections.stage ? 'rotate-180' : ''}`}>
            ▼
          </span>
        </button>

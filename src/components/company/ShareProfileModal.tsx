...
export function ShareProfileModal({ companyId, companyName, isOpen, onClose }: ShareProfileModalProps) {
  ...
  const inputClass = 'w-full px-3 py-2 bg-[var(--bg-elev)] border border-[var(--line-strong)] rounded-lg text-[var(--ink)] text-sm placeholder-gray-400 focus:outline-none focus:shadow-[var(--focus-ring)] focus:border-[var(--line-strong)]'
      ...
      <div className="relative bg-[var(--bg-elev)] rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="space-y-3">
            ...
            <div>
              <label className="block text-xs font-medium text-[var(--ink-soft)] mb-1">Recipient email *</label>
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="investor@fund.com" className={inputClass} />
            </div>
            <div>
              <label className="block text-xs font-medium text-[var(--ink-soft)] mb-1">Recipient name</label>
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Jane Smith (optional)" className={inputClass} />
            </div>
            <div>
              <label className="block text-xs font-medium text-[var(--ink-soft)] mb-1">Add a note for the recipient</label>
            ...
            <div className="flex items-center gap-4">
              <label className="flex items-center gap-2 text-sm text-[var(--ink-soft)] cursor-pointer">
                <input type="checkbox" checked={includeDealRoom} onChange={(e) => setIncludeDealRoom(e.target.checked)} className="rounded border-[var(--line-strong)]" />
                Include Deal Room access
              </label>
              <label className="flex items-center gap-2 text-sm text-[var(--ink-soft)] cursor-pointer">
                <input type="checkbox" checked={includeScore} onChange={(e) => setIncludeScore(e.target.checked)} className="rounded border-[var(--line-strong)]" />
                Include Kunfa Score
              </label>
            </div>
          ...
          {(shares.length > 0 || loadingShares) && (
            <div className="border-t border-[var(--line)] pt-4">
              <h3 className="text-xs font-semibold text-[var(--ink-mute)] uppercase tracking-wider mb-3">Shared with</h3>
              {loadingShares ? (
                <div className="py-4 text-center"><Loader2 className="w-5 h-5 text-[var(--ink-faint)] animate-spin mx-auto" /></div>
              ) : (
                <div className="space-y-2">
                  {shares.map((s) => {
                    ...
                    return (
                      <div key={s.id} className={`flex items-center gap-3 p-2.5 rounded-lg border ${revoked || expired ? 'bg-[var(--bg-sunk)] border-[var(--line)]' : 'bg-[var(--bg-elev)] border-[var(--line)]'}`}>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-[var(--ink)] truncate">
                            {s.recipient_name || s.recipient_email}
                          </p>
                          {s.recipient_name && (
                            <p className="text-xs text-[var(--ink-mute)] truncate">{s.recipient_email}</p>
                          )}
                          <div className="flex items-center gap-3 mt-0.5 text-xs text-[var(--ink-faint)]">
                        ...
                        {s.is_active && !expired && (
                          <button
                            onClick={() => revoke(s.id)}
                            className="p-1.5 text-[var(--ink-faint)] hover:text-red-500 transition rounded"
                            title="Revoke access"
                          >

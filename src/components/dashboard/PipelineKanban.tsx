    ...
    <div className="bg-[var(--bg-elev)] rounded-lg border border-[var(--line)] p-4 mb-3  ">
      ...
      <div className="flex items-start justify-between mb-2">
        <div className="flex-1 min-w-0">
          <h4 className="font-semibold text-[var(--ink)] text-sm truncate">
            {deal.deals?.company_name || 'Unknown Company'}
          </h4>
          {deal.deals?.industry && (
            <p className="text-xs text-[var(--ink-mute)] mt-1">{deal.deals.industry}</p>
          )}
        </div>
        <button
          onClick={() => setShowMenu(!showMenu)}
          className="p-1 hover:bg-[var(--bg-sunk)] rounded transition-colors"
        >
          <MoreHorizontal className="h-4 w-4 text-[var(--ink-faint)]" />
        </button>
      </div>
      ...
      {/* Notes Preview */}
      {notePreview && (
        <p className="text-xs text-[var(--ink-soft)] mb-3 line-clamp-2">{notePreview}</p>
      )}
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="p-1.5 rounded bg-[var(--bg-sunk)] hover:bg-[var(--bg-sunk)] disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          title="Move to previous stage"
        >
          <ChevronLeft className="h-3 w-3 text-[var(--ink-soft)]" />
        </button>
        <span className="text-xs text-[var(--ink-mute)] font-medium flex-1 text-center">
        ...
        >
          <ChevronRight className="h-3 w-3 text-[var(--ink-soft)]" />
        </button>
      </div>

      {/* Dropdown Menu */}
      {showMenu && (
        <div className="absolute right-4 mt-2 w-48 bg-[var(--bg-elev)] rounded-lg border border-[var(--line)] z-10">
          <button className="w-full px-4 py-2 text-sm text-[var(--ink-soft)] hover:bg-[var(--bg-sunk)] flex items-center gap-2 text-left">
          ...
          <button
            ...
            className="w-full px-4 py-2 text-sm text-red-600 hover:bg-red-50 flex items-center gap-2 text-left border-t border-[var(--line)]"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {STAGES.map((stage) => (
          <div key={stage.id} className="space-y-3">
            <div className="h-8 bg-[var(--bg-sunk)] rounded animate-pulse" />
            {[...Array(2)].map((_, i) => (
              <div key={i} className="h-48 bg-[var(--bg-sunk)] rounded animate-pulse" />
            ))}
          </div>
        ))}
      </div>
      ...
      {STAGES.map((stage) => {
        ...
        return (
          <div key={stage.id} className={`${stage.color} rounded-lg border-2 ${stage.borderColor} p-4 min-h-96`}>
            {/* Stage Header */}
            <div className="mb-4">
              <h3 className="font-bold text-[var(--ink)]">{stage.label}</h3>
            ...
            <div className="space-y-3 max-h-96 overflow-y-auto">
              {stageDeals.length === 0 ? (
                <div className="text-center py-8 text-[var(--ink-mute)]">

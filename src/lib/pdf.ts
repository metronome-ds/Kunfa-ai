...
// Color Scheme — Kunfa blue primary
const blue = '#0168FE'
const navy = '#0F172A'
const gray = '#64748B'
...
// Logo path — resolved at build time for server-side rendering
const logoPath = path.join(process.cwd(), 'public', 'images', 'kunfa-logo.png')
...
const styles = StyleSheet.create({
  ...
  coverSubtitle: { fontSize: 13, color: '#94A3B8', marginBottom: 50, textAlign: 'center' },
  coverCompanyName: { fontSize: 22, fontWeight: 'bold', color: blue, marginBottom: 40, textAlign: 'center' },
  coverScore: { fontSize: 80, fontWeight: 'bold', color: blue, marginBottom: 6 },
  coverScoreLabel: { fontSize: 13, color: '#94A3B8', marginBottom: 40, letterSpacing: 1 },
  coverDate: { fontSize: 11, color: '#64748B' },
  coverConfidential: { fontSize: 9, color: '#475569', marginTop: 20, letterSpacing: 4 },
  header: {
    flexDirection: 'row',
  ...
  headerLogo: { width: 80, height: 20, objectFit: 'contain' },
  ...
  footerLogo: { width: 50, height: 13, objectFit: 'contain' },
  footerText: { fontSize: 8, color: gray },
  footerPage: { fontSize: 8, color: gray },
...
function PageHeader() {
  return React.createElement(View, { style: styles.header },
    React.createElement(Image, { src: logoPath, style: styles.headerLogo }),
...
function PageFooter({ pageNum }: { pageNum: number }) {
  return React.createElement(View, { style: styles.footer },
    React.createElement(Image, { src: logoPath, style: styles.footerLogo }),
    React.createElement(Text, { style: styles.footerText }, 'Confidential \u2014 Prepared by Kunfa.AI'),
    React.createElement(Text, { style: styles.footerPage }, `Page ${pageNum}`)
  )
}
...
export async function generateReport(
  ...
  const companyName = result.company_profile?.company_name || ''
  ...
  const doc = React.createElement(Document, {},
    ...
    React.createElement(Page, { size: 'A4', style: styles.coverPage },
      // Logo image
      React.createElement(Image, { src: logoPath, style: styles.coverLogo }),
      // Title
      ...
      // Company name
      ...(companyName
        ? [React.createElement(Text, { key: 'company-name', style: styles.coverCompanyName }, companyName)]
        : []),
      // Score
    ...
    React.createElement(Page, { size: 'A4', style: styles.page },
      ...
      React.createElement(View, { style: { ...styles.badge, backgroundColor: '#ECFDF5' } },
        React.createElement(Text, { style: { fontSize: 9, fontWeight: 'bold', color: blue } },
      ...
      ...(['team', 'market', 'product', 'financial'] as const).flatMap((dimKey) => {
        ...
        return [
          React.createElement(View, { key: `${dimKey}-header`, style: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 } },
            ...
            React.createElement(Text, { style: { fontSize: 11, fontWeight: 'bold', color: blue } }, `${dimScore}/25 (${dimGrade})`)
          ),
          ...paragraphs.map((p, i) =>
    ...
    React.createElement(Page, { size: 'A4', style: styles.page },
      ...
      React.createElement(View, { style: styles.swotGrid },
        React.createElement(View, { style: { ...styles.swotQuadrant, backgroundColor: '#ECFDF5', borderLeftWidth: 3, borderLeftColor: blue } },
          React.createElement(Text, { style: styles.swotTitle }, 'Strengths'),
          ...memo.swot.strengths.map((item) =>
            BulletItem({ text: item, color: blue })
          )
        ),
        ...
        React.createElement(View, { style: { ...styles.swotQuadrant, backgroundColor: '#EEF2FF', borderLeftWidth: 3, borderLeftColor: indigo } },
          ...
          ...memo.swot.opportunities.map((item) =>
    ...
    React.createElement(Page, { size: 'A4', style: styles.page },
      ...
      ...memo.fundraising_readiness.next_steps.map((step) =>
        BulletItem({ text: step, color: blue })
      ),
      React.createElement(View, { style: styles.divider }),
      ...
      React.createElement(Text, { style: { textAlign: 'center', fontSize: 9, color: blue, marginTop: 6, fontWeight: 'bold' } },
        'Kunfa.AI \u2014 Venture Intelligence'
      ),
